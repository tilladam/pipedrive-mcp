"""Tool for getting a comment on a note from Pipedrive."""
...
@tool("notes")
async def get_comment_on_note_from_pipedrive(
    ctx: Context,
    note_id: str,
    comment_id: str,
) -> str:
    """Get a specific comment on a note from Pipedrive.

    Retrieves a single comment by its ID from a specific note.
    ...
    Usage example:
    get_comment_on_note_from_pipedrive(note_id="123", comment_id="456")
    ...
    Returns:
        JSON string with success status and comment data, or error message
    """
    try:
        ...
        result = await client.notes.comments.get_comment(
            note_id=note_id_int,
        ...
        error_msg = f"Unexpected error getting comment: {str(e)}"
